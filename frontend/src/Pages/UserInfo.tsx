...
export function UserInfo(props:UserInfoStyle)
{
    ...
    async function HitBackend()
    {
        ...
        {
            hideLoading();
            showError("Error while unfriending a user!");
        }        
    } 
    return<>
        {!UnfreindUser
        ?<div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center p-4">
            <div className=" bg-black-500 w-[95%] max-w-[30rem] rounded-xl border-slate-300 border flex flex-col">
               ...
               <div className="flex justify-center items-center w-full mt-[1.5rem] px-[2rem]">
                    <button type="button" aria-label="Name" className="flex justify-center items-center text-white font-bold border border-red-500 rounded-md pl-[2rem] pr-[2rem] lg:pl-[5rem] lg:pr-[5rem] bg-red-600 text-[0.8rem]" onClick={() => HitBackend()}><div><UnfriendOrLeaveGroup/></div><div>Unfriend this User</div></button>
                </div>
                <div className="text-red-700 flex justify-center items-center text-[0.8rem] mt-[0.5rem] text-center font-bold pb-[1.5rem] px-[1rem]">You may have to send request to this user again for messaging!</div>
            </div>
        </div>
        :<div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center p-4">
            <div className=" bg-black-500 w-[95%] max-w-[30rem] rounded-xl border-slate-300 border flex flex-col">
                ...
                <div className="w-full h-[1px] bg-slate-500 mt-[2rem]"></div>
               <div className="flex justify-center items-center w-full mt-[1.5rem] pb-[1.5rem]">
                    <button type="button" aria-label="Name" className=" h-[3rem] flex justify-center items-center text-white font-bold border border-black-800 rounded-md pl-[5rem] pr-[5rem] bg-green-900 text-[0.8rem]" onClick={()=>props.SetUserSelector()}><div>Continue to Profile</div></button>
                </div>
            </div>
        </div>}
    </>
