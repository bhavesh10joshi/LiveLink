...
export function GroupInfo(props:GroupInfoStyle)
{
    ...
    useEffect(function(){
        const HitGroupDetailsBackend = async () => {
            ...
            catch(e)
            {
                ...
                setIsLoading(false);
                showError("Error occurred while fetching group info!");
                return;
            }         
        };
    ...
    async function RemoveMember(UserUniqueId : any)
    {
        ...
        try
        {
            ...
            hideLoading();
            showSuccess("Removed the user successfully!");
            return;
        }
        catch(e)
        {
            hideLoading();
            showError("Error occurred while removing the member from the group!");
            return;
        }
    }
    return<>
        ...
        : !EditGroupinfo ? !LeaveGroup ?<div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center p-4">
            <div className=" bg-black-500 w-[95%] max-w-[30rem] max-h-[90vh] rounded-xl border-slate-300 border flex flex-col overflow-y-auto">
                ...
                {isLoading
                    ? <div className="flex flex-col items-center mt-[2rem] gap-4 px-[2rem]">
                        <div className="animate-pulse bg-gray-700/50 rounded-full w-[10rem] h-[10rem]"></div>
                        <div className="animate-pulse bg-gray-700/50 rounded h-6 w-40"></div>
                        <div className="animate-pulse bg-gray-700/50 rounded h-4 w-60"></div>
                        <div className="w-full mt-4 flex flex-col gap-3"><SkeletonProfile /><SkeletonProfile /><SkeletonProfile /></div>
                      </div>
                    : <>
